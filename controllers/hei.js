...
module.exports = {
  index: async(req,res) => {
    try {
      ...
      returnMessage.successMessage(res,messages.successMessages.getAllheis,heis);
    } catch (error) {
  ...
  update: async(req,res) => {
    try {
      const hei = await heiModel.findByIdAndUpdate(req.params['id'], { ...req.body }, {new: true});
      await hei.populate(heiPopulate)
      returnMessage.successMessage(res,messages.successMessages.updateHei, hei);
    } catch (error) {
      returnMessage.errorMessage(res,error);
    }
  },
