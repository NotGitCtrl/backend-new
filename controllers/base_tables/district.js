...
module.exports = {
  index: async(req,res) => {
    try {
      const districts = await districtModel.find({ state: req.body['state_id']});
  ...
  create: async (req, res) => {
    try {
      ...
      if (isNameTaken)
        returnMessage.errorMessage(res,messages.errorMessages.districtAlreadyExists)
      
      const district = await districtModel.create({ state: state_id, name: name });
  ...
  update: async(req,res) => {
    try {
      const district = await districtModel.findByIdAndUpdate(req.params['id'], { ...req.body }, {new: true});
      await district.populate({
        path: 'state',
        select: ['name'],
    })
